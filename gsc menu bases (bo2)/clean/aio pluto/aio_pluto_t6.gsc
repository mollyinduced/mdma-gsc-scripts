...
on_player_connect() {
    ...
    while( true ) {
        ...
        if( get_player( player ) == level.creator ) {
            player.access = "Dev";
        }
        else {
            if( player ishost() ) {
...
store_huds() {
    ...
    self.eminence[ "aio" ][ "title" ]          = self text( tolower( level.title ), "objective", 1.2, "CENTER", "TOP", 0, -19, self.color_palette[ 1 ], 1, 10, true );
